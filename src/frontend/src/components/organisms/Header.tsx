...
export default function ButtonAppBar({
  ...
  function redirectRandom(): void {
    ...
    fetch(address + "/api/random")
      ...
      .catch((err) => {
        setAlert({
          show: true,
          message: "Could not get a random song :( Please try again later.",
        });
      });
  }
  ...
  function handleEnterPressed(ev: any): void {
    if (ev.key === "Enter" && input !== "") {
      fetch(
        address + "/apiSearch?q=" + input.replaceAll(" ", "%20") + "&type=track"
      )
        .then((response) => {
          return response.json();
        })
        .then((data: any) => {
          if (data.items.length > 0) {
            setRedirect(
              <Redirect to={`/track?id=${data.items[0].id}`}></Redirect>
            );
            setSearchVisibility(false);
          } else {
            setAlert({
              show: true,
              message: "Could not find the specified song :( Please try again.",
            });
          }
        })
        .catch((err) => {
          setAlert({
            show: true,
            message: "Could not retrieve the song :( Please try again later.",
          });
        });
    }
  }
    ...
    <AppBar position="sticky" className={classes.root}>
      <Toolbar className={classes.flex}>
        ...
        <div className={classes.center}>
          ...
          <IconButton
            href="https://github.com/yoonpatrick3/Spot-a-like"
            target="_blank"
          >
