...
import StatTooltip from "../../../organisms/StatTooltip";
import StatDialog from "../../../organisms/StatDialog";
...
const Search = ({
      ...
      .then((data) => {
        if (option === "artist") {
          let artist_array = data.items.map((artist: ArtistJSON) => {
            ...
            return (
              <SongCard
                type={SongCardType.Artist}
                id={artist.id}
                trackArtist={artist.artist_name}
                imageURL={url}
                trackName=""
              ></SongCard>
            );
  ...
  return (
    <div className="search">
      ...
      <div>
        <h2>
          Search by vibes <StatTooltip setOpen={setOpen} />{" "}
        </h2>
        <ContinuousSlider
